...
function CoursePlannerModal({ open, coursePlan, draggedCourse, onDragStart, onDrop, onClose }: { open: boolean; coursePlan: PlannerSchedule; draggedCourse: string | null; onDragStart: (course: string) => void; onDrop: (course: string, term: string) => void; onClose: () => void }) {
  ...
  return <div className="planner-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="course-planner planner-modal" role="dialog" aria-modal="true" aria-labelledby="course-planner-title">
    <div className="planner-heading"><div><p className="eyebrow">COURSE CONFIGURATOR</p><h3 id="course-planner-title">把課程拖到你想修的學期</h3></div><div className="planner-actions"><p>這是個人規劃草稿，會只儲存在這台裝置的瀏覽器；不會更改學校系統或 iPOS。</p><button type="button" className="modal-close" onClick={onClose} aria-label="關閉課程配置器">關閉 ×</button></div></div>
    ...
    <main>
      ...
      <div className="page app-shell" id="top">
        {view === "home" && <section className="workspace-panel calendar-panel" aria-labelledby="calendar-title">
          <div className="workspace-heading"><div><p className="eyebrow">WEEK AT A GLANCE</p><h2 id="calendar-title">這週哪一天有什麼</h2></div><p>紅色是硬截止；灰色是尚未決定是否出席。9/10 下午有重疊，SER 334 應優先。</p></div>
          <div className="week-calendar" aria-label="2026 年 9 月 9 日到 15 日行事曆">{CALENDAR_DAYS.map((day) => <article className={"calendar-day" + (day.today ? " is-today" : "")} key={day.weekday}><header><span>{day.weekday}</span><strong>{day.date}</strong>{day.today && <small>TODAY</small>}</header><div>{day.items.map((item) => <div className={"calendar-item " + (item.tone ?? "class")} key={item.title}><strong>{item.title}</strong><span>{item.detail}</span></div>)}</div></article>)}</div>
        </section>}

        {view === "tasks" && <section className="workspace-panel" aria-labelledby="tasks-title"><div className="workspace-heading"><div><p className="eyebrow">OPERATING BOARD</p><h2 id="tasks-title">待辦依類別收好</h2></div><p>{completed}/{TASKS.length} 完成。點每個項目的「細節與資源」才展開說明，不必一直看長文字。</p></div><div className="task-group-grid">{TASK_GROUPS.map((group) => <article className="task-group" key={group.title}><h3>{group.title}</h3>{group.ids.map((id) => { const item = taskById(id); return item ? <article className={"compact-task " + (checked[item.id] ? "is-done" : "")} key={item.id}><label><input type="checkbox" checked={Boolean(checked[item.id])} onChange={() => toggle(item.id)} /><span className="fake-check" aria-hidden="true">✓</span><span><small>{item.tag}</small><strong>{item.label}</strong></span></label><details><summary>細節與資源</summary><p>{item.note}</p><div>{item.resources.map((resource) => <a href={resource.href} target="_blank" rel="noreferrer" key={resource.label}>{resource.label} ↗</a>)}</div></details></article> : null; })}</article>)}</div></section>}
        ...
        {view === "immigration" && <section className="workspace-panel" aria-labelledby="immigration-title">
          ...
          <section className="stage-selector" aria-labelledby="stage-selector-title">
            <div><p className="mini-label">你的目前階段</p><h3 id="stage-selector-title">從哪一段開始看？</h3><p>選擇後，下方只保留這個階段與之後的行動、風險和備案。</p></div>
        ...
        <footer><p>Ian · ASU Fall 2026 Dashboard</p><p>非敏感摘要；課程狀態以 Canvas 個別作業／測驗頁為準。最後更新：2026/09/10。</p></footer>
      </div>
    </main>
  );
}
